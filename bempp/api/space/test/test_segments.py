...
@pytest.mark.parametrize('space_info', [("P", 1), ("DUAL", 0), ("RWG", 0), ("SNC", 0)])
...
def test_segments_dual1_space(space_info, helpers, precision):
    ...
    fun1 = bempp.api.GridFunction(space0, coefficients=_np.ones(space0.global_dof_count))
    ...
    fun2 = bempp.api.GridFunction(space1, coefficients=_np.ones(space1.global_dof_count))

    space3 = bempp.api.function_space(
        grid, space_info[0], space_info[1], segments=[1],
        include_boundary_dofs=True,
        truncate_at_segment_edge=False
    )
    fun3 = bempp.api.GridFunction(space2, coefficients=_np.ones(space2.global_dof_count))

    assert space0.global_dof_count < space2.global_dof_count
    assert space0.global_dof_count == space1.global_dof_count
    assert space2.global_dof_count == space3.global_dof_count
    assert 0 < fun0.l2_norm() < fun1.l2_norm() < fun3.l2_norm()
    assert 0 < fun0.l2_norm() < fun2.l2_norm() < fun3.l2_norm()
