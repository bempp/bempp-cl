...
evaluate_scalar_potential_novec(__global REALTYPE *grid,
  ...
  __local REALTYPE16 localResult[WORKGROUP_SIZE];
  REALTYPE16 myResult = M_ZERO;
#else
  ...
  REALTYPE16 myCoefficients[NUMBER_OF_SHAPE_FUNCTIONS];
  for (int index = 0; index < NUMBER_OF_SHAPE_FUNCTIONS; ++index)
    myCoefficients[index] = (REALTYPE16)(
