...
__kernel void evaluate_singular(
  ...
  for (collocationIndex = 0; collocationIndex < NUMBER_OF_TEST_SHAPE_FUNCTIONS; ++collocationIndex)
  {
      ...
      for (uint quadIndex = localQuadPointsPerItem * localId;
          ...
          {
#ifndef COMPLEX_KERNEL
            result[collocationIndex][j] += trialValue[j] * weight * kernelValue;
#else
            result[collocationIndex][j][0] +=
                trialValue[j] * weight * kernelValue[0];
            result[collocationIndex][j][1] +=
                trialValue[j] * weight * kernelValue[1];
#endif
          }
      }
  }
  ...
  for (i = 0; i < NUMBER_OF_TEST_SHAPE_FUNCTIONS; ++i)
    for (j = 0; j < NUMBER_OF_TRIAL_SHAPE_FUNCTIONS; ++j) {
#ifndef COMPLEX_KERNEL
      localResult[localId][i][j] = result[i][j] * trialIntElem;
#else
      localResult[localId][i][j][0] =
          result[i][j][0] * trialIntElem;
      localResult[localId][i][j][1] =
          result[i][j][1] * trialIntElem;
#endif
    }
