...
__kernel __attribute__((vec_type_hint(REALTYPE4))) void evaluate_dense_regular(
    ...
    REALTYPE4 firstTermIntegral;
    REALTYPE4 tempFirstTerm;
    ...
    REALTYPE4 firstTermIntegral[2];
    REALTYPE4 tempFirstTerm[2];
    ...
    for (vecIndex = 0; vecIndex < 4; ++vecIndex)
        if (!elementsAreAdjacent(testElement, trialElement[vecIndex], gridsAreDisjoint)) {
            ...
            for (i = 0; i < NUMBER_OF_TEST_SHAPE_FUNCTIONS; ++i)
                for (j = 0; j < NUMBER_OF_TRIAL_SHAPE_FUNCTIONS; ++j)
                    localResult[4 * lid + vecIndex][i][j] = ((REALTYPE*)(&shapeIntegral[i][j]))[vecIndex] *
                                                            testIntElem * ((REALTYPE*)(&trialIntElem))[vecIndex] * localCoeffs[j];
        }
        else {
            for (i = 0; i < NUMBER_OF_TEST_SHAPE_FUNCTIONS; ++i)
                for (j = 0; j < NUMBER_OF_TRIAL_SHAPE_FUNCTIONS; ++j)
                    localResult[4 * lid + vecIndex][i][j] = M_ZERO;
        }
#else
    for (vecIndex = 0; vecIndex < 4; ++vecIndex)
        if (!elementsAreAdjacent(testElement, trialElement[vecIndex], gridsAreDisjoint)) {
            ...
            for (i = 0; i < NUMBER_OF_TEST_SHAPE_FUNCTIONS; ++i)
                for (j = 0; j < NUMBER_OF_TRIAL_SHAPE_FUNCTIONS; ++j) {
                    localResult[4 * lid + vecIndex][i][j][0] = testIntElem * ((REALTYPE*)(&trialIntElem))[vecIndex] *
                            (((REALTYPE*)(&shapeIntegral[i][j][0]))[vecIndex] * localCoeffs[j][0] - ((REALTYPE*)(&shapeIntegral[i][j][1]))[vecIndex] * localCoeffs[j][1]);
                    localResult[4 * lid + vecIndex][i][j][1] = testIntElem * ((REALTYPE*)(&trialIntElem))[vecIndex] *
            ...
            for (i = 0; i < NUMBER_OF_TEST_SHAPE_FUNCTIONS; ++i)
                for (j = 0; j < NUMBER_OF_TRIAL_SHAPE_FUNCTIONS; ++j) {
                    localResult[4 * lid + vecIndex][i][j][0] = M_ZERO;
                    localResult[4 * lid + vecIndex][i][j][1] = M_ZERO;
                }
