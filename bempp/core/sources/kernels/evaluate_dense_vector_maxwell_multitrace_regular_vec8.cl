...
__kernel void evaluate_dense_maxwell_multitrace_vector_regular(
    ...
    REALTYPE8 tempResultElectricFirstComponent[3][3][2];
    REALTYPE8 tempResultElectricSecondComponent[2];
    REALTYPE8 tempResultMagnetic[3][3][2];
    REALTYPE8 shapeIntegralElectricFirstComponent[3][3][2];
    REALTYPE8 shapeIntegralElectricSecondComponent[2];
    REALTYPE8 shapeIntegralElectric[3][3][2];
    REALTYPE8 shapeIntegralMagnetic[3][3][2];
    ...
    REALTYPE8 tempFactorInt[2];
    REALTYPE8 tempResultElectricFirstComponentInt[3][3][2];
    REALTYPE8 tempResultElectricSecondComponentInt[2];
    REALTYPE8 tempResultMagneticInt[3][3][2];
    REALTYPE8 shapeIntegralElectricFirstComponentInt[3][3][2];
    REALTYPE8 shapeIntegralElectricSecondComponentInt[2];
    REALTYPE8 shapeIntegralElectricInt[3][3][2];
    REALTYPE8 shapeIntegralMagneticInt[3][3][2];
    REALTYPE shiftedWavenumberInt[2] = {M_ZERO, M_ZERO};
    REALTYPE inverseShiftedWavenumberInt[2] = {M_ZERO, M_ZERO};
    REALTYPE8 product2[2];
...
// Computation of 1i * wavenumber and 1 / (1i * wavenumber)
    ...
    for (testQuadIndex = 0; testQuadIndex < NUMBER_OF_QUAD_POINTS;
        ...
        for (trialQuadIndex = 0; trialQuadIndex < NUMBER_OF_QUAD_POINTS;
            ...
            getPiolaTransformVec8(trialIntElem, trialJac, trialValue, trialElementValue);
    ...
    for (vecIndex = 0; vecIndex < 8; vecIndex++)
        if (!elementsAreAdjacent(testElement, trialElement[vecIndex], gridsAreDisjoint)) {
            ...
            for (i = 0; i < 3; ++i)
                for (j = 0; j < 3; ++j) {
                    localResultElectric[8 * lid + vecIndex][i][j][0] =
                    ...
                    localResultElectric[8 * lid + vecIndex][i][j][1] =
                        testIntElem * VEC_ELEMENT(trialIntElem, vecIndex) *
                    ...
                    localResultMagnetic[8 * lid + vecIndex][i][j][0] =
                        testIntElem * VEC_ELEMENT(trialIntElem, vecIndex) *
                    ...
                    localResultMagnetic[8 * lid + vecIndex][i][j][1] =
                        testIntElem * VEC_ELEMENT(trialIntElem, vecIndex) *
            ...
            for (i = 0; i < 3; ++i)
                for (j = 0; j < 3; ++j) {
                    localResultElectric[8 * lid + vecIndex][i][j][0] = M_ZERO;
                    localResultElectric[8 * lid + vecIndex][i][j][1] = M_ZERO;

                    localResultMagnetic[8 * lid + vecIndex][i][j][0] = M_ZERO;
                    localResultMagnetic[8 * lid + vecIndex][i][j][1] = M_ZERO;
                }
        }
