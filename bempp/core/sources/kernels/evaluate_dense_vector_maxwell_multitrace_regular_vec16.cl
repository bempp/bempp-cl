...
__kernel void evaluate_dense_maxwell_multitrace_vector_regular(
    ...
    REALTYPE16 tempResultElectricFirstComponent[3][3][2];
    REALTYPE16 tempResultElectricSecondComponent[2];
    REALTYPE16 tempResultMagnetic[3][3][2];
    REALTYPE16 shapeIntegralElectricFirstComponent[3][3][2];
    REALTYPE16 shapeIntegralElectricSecondComponent[2];
    REALTYPE16 shapeIntegralElectric[3][3][2];
    REALTYPE16 shapeIntegralMagnetic[3][3][2];
    ...
    REALTYPE16 tempFactorInt[2];
    REALTYPE16 tempResultElectricFirstComponentInt[3][3][2];
    REALTYPE16 tempResultElectricSecondComponentInt[2];
    REALTYPE16 tempResultMagneticInt[3][3][2];
    REALTYPE16 shapeIntegralElectricFirstComponentInt[3][3][2];
    REALTYPE16 shapeIntegralElectricSecondComponentInt[2];
    REALTYPE16 shapeIntegralElectricInt[3][3][2];
    REALTYPE16 shapeIntegralMagneticInt[3][3][2];
    REALTYPE shiftedWavenumberInt[2] = {M_ZERO, M_ZERO};
    REALTYPE inverseShiftedWavenumberInt[2] = {M_ZERO, M_ZERO};
    REALTYPE16 product2[2];
...
// Computation of 1i * wavenumber and 1 / (1i * wavenumber)
    ...
    for (testQuadIndex = 0; testQuadIndex < NUMBER_OF_QUAD_POINTS;
        ...
        for (trialQuadIndex = 0; trialQuadIndex < NUMBER_OF_QUAD_POINTS;
            ...
            getPiolaTransformVec16(trialIntElem, trialJac, trialValue, trialElementValue);
    ...
    for (vecIndex = 0; vecIndex < 16; vecIndex++)
        if (!elementsAreAdjacent(testElement, trialElement[vecIndex], gridsAreDisjoint)) {
            ...
            for (i = 0; i < 3; ++i)
                for (j = 0; j < 3; ++j) {
                    localResultElectric[16 * lid + vecIndex][i][j][0] =
                    ...
                    localResultElectric[16 * lid + vecIndex][i][j][1] =
                        testIntElem * VEC_ELEMENT(trialIntElem, vecIndex) *
                    ...
                    localResultMagnetic[16 * lid + vecIndex][i][j][0] =
                        testIntElem * VEC_ELEMENT(trialIntElem, vecIndex) *
                    ...
                    localResultMagnetic[16 * lid + vecIndex][i][j][1] =
                        testIntElem * VEC_ELEMENT(trialIntElem, vecIndex) *
            ...
            for (i = 0; i < 3; ++i)
                for (j = 0; j < 3; ++j) {
                    localResultElectric[16 * lid + vecIndex][i][j][0] = M_ZERO;
                    localResultElectric[16 * lid + vecIndex][i][j][1] = M_ZERO;

                    localResultMagnetic[16 * lid + vecIndex][i][j][0] = M_ZERO;
                    localResultMagnetic[16 * lid + vecIndex][i][j][1] = M_ZERO;
                }
        }
