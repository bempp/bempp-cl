...
@pytest.mark.parametrize('space_info', [("DP", 0), ("DP", 1), ("P", 1), ("DUAL", 0), ("DUAL", 1),
    ...
    for n, i in enumerate(grid.domain_indices):
        if i != 1:
            for cell in range(c * n, c * (n + 1)):
                assert math.isclose(_np.linalg.norm(evals[:, cell]), 0,
...
@pytest.mark.parametrize('space_info', [("P", 1), ("DUAL", 0), ("DUAL", 1),
                                        ("RWG", 0), ("SNC", 0), ("BC", 0), ("RBC", 0)])
